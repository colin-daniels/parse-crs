...
impl Deserialize for Operator {
    ...
    fn deserialize(input: Pair<Rule>) -> Result<Self, Self::Error> {
        ...
        let negate = inner.next_if_rule(Rule::test_negate).is_some();
        let op = inner.next().unwrap().as_str();
        let op = OperatorType::from_name(op)
            .ok_or_else(|| OperatorParseError::UnknownOperator(op.to_owned()))?;
        let arg = inner.next().map(|p| p.as_str().to_owned());

        Ok(Self { negate, op, arg })
    }
}
...
impl Serialize for Operator {
    ...
    fn serialize<W: Write>(&self, mut out: W) -> Result<(), Self::Error> {
        ...
        match &self.arg {
            Some(arg) => write!(out, "{negate}@{op} {arg}"),
            None => write!(out, "{negate}@{op}"),
        }
    }
}
